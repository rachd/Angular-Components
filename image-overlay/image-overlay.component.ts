...
*     As the user scrolls, the overlay will linear wipe in.
...
@Component({
  ...
  template: `<img class="image" src="{{image}}"/>
            <div class="overlay" [ngStyle]="{'background-image': 'url(' + overlay + ')', 'height': percentShowing + '%'}"></div>`,
...
export class ImageOverlayComponent implements OnInit {
    ...
    percentScrolledIntoView(el) {
        ...
        if (amountTopShowing < START_SHOWING) {
            ...
            return (amountTopShowing - START_SHOWING) / elemHeight * 100;
        } else {
            return 100;
        }
    }
